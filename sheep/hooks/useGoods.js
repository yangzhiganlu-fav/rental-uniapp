    ...
    activity.rules.forEach((rule) => {
        const conditionTypeStr =
            activity.conditionType === 10
                ? `满 ${fen2yuanSimple(rule.limit)} 元`
                : `满 ${rule.limit} 件`;
        // 满减
        if (rule.limit) {
            result[0].values.push(
                `${conditionTypeStr} 减 ${fen2yuanSimple(rule.discountPrice)} 元`,
            );
        }
        // 赠品
        if (
            rule.point ||
            (rule.giveCouponTemplateCounts && rule.giveCouponTemplateCounts.length > 0)
        ) {
            let tips = [];
            if (rule.point) {
