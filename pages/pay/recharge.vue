    ...
    <s-layout title="充值" class="withdraw-wrap" navbar="inner">
        ...
        >
            ...
            <button
                ...
                @tap="sheep.$router.go('/pages/pay/recharge-log')"
            >
                充值记录
            </button>
        </view>
            ...
            <view class="recharge-card-box">
                ...
                <view class="face-value-box ss-flex ss-flex-wrap ss-m-y-40">
                    <button
                        ...
                        :key="item.money"
                        :class="[
                            { 'btn-active': state.recharge_money === fen2yuan(item.payPrice) },
                        ]"
...
<script setup>
    ...
    async function onConfirm() {
        const { code, data } = await PayWalletApi.createWalletRecharge({
            packageId: state.packageList.find(
                (item) => fen2yuan(item.payPrice) === state.recharge_money,
            )?.id,
